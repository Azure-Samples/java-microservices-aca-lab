...
Now that you’ve familiarized yourself with the example Spring Petclinic Microservices application that you’ll be deploying, you can explore the Azure services that you’ll use to host it.
...
In this lab, we’ll use Azure Container Apps to host the various components that you reviewed in the previous section. Our API gateway application will handle external user requests, routing them to the appropriate back-end services, which are each running in their own container app.

Using Azure Container Apps, you’ll be able to host the application’s entire microservices architecture with a minimal amount of management work. This allows you to focus on the applications while Azure Container Apps takes care of all the managed Spring components.

The following diagram shows how the various Spring Boot microservice components will interact within the Azure Container Apps host once you deploy them.
...
-   [Azure Container Registry](https://learn.microsoft.com/azure/container-registry/container-registry-intro), which is a managed registry service based on the open-source Docker Registry 2.0. You’ll use it to build and save the images that Azure Container Apps uses to create the various containerized services.
