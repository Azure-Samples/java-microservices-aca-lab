...
1. Once the app **api-gateway** deployment has succeeded, execute the same statements for the other microservices **customers-service**, **vets-service** and **visits-service**.
   ...
   {: .note }
   > The script [../tools/update-apps-appinsights.sh](https://github.com/Azure-Samples/java-microservices-aca-lab/blob/main/tools/update-apps-appinsights.sh) include all the steps to update the target apps, and it run the commands parallelly to save the lab execution time. Open the file for more details.
