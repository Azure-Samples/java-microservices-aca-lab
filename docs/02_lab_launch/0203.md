...
You now have the compute service in place to host your applications. Before you start deploying individual microservices as container apps, you need to first create an Azure Database for MySQL - Flexible Server–hosted database for them.

During creation, you’ll configure the MySQL database to have a firewall enabled. By default, this firewall will block all incoming calls. To allow your microservices running in the Azure Container Apps service to access the database, you’ll need to add firewall rules.
