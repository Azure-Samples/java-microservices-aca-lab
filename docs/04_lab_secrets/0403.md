...
NOTE TO THE TESTERS OF THE LAB: I think the full datasource stuff can be removed from the application.yml file, since this now comes from the service connection. Can you test and double check. If it works, we need to also update the [0403_application.yml file](0403_application.yml)

1. In the config repository you will need to update the database connection information. Replace the contents of the current `application.yml` file with the contents of the [0403_application.yml file](0403_application.yml). Make sure you fill out your current MySQL server name on line `12`. This file includes the following changes:

   - It adds a `serverTimezone=UTC` on line `12`.
   - It changes the username to `aad_mysql_conn` on line `13`, which is the name of the AAD connected user you created in the database.
